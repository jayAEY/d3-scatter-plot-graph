...
d3.json('https://raw.githubusercontent.com/freeCodeCamp/ProjectReferenceData/master/cyclist-data.json')
    .then((data)=>{
        ...
        data.forEach((d)=>{
            d.Time = new Date(1,1,1,0,d.Time.split(':')[0],d.Time.split(':')[1]);
        ...
        const mouseOver = (e,d) => {
            tooltip.transition()
                   ...
                   .style('margin-left',`${e.pageX - 300}px`)
                   .style('margin-top',`${e.pageY - 620}px`)
                   .style('background-color',`${ d.Doping ? 'red' : '#0ead6d'}`)
                   .style('color',`${ d.Doping ? 'white' : 'white'}`)

            tooltip.text(`${d.Name} ${d.Doping ? d.Year + ': ' : d.Year} ${d.Doping} `)
